...
describe Notu::MostPlayedTracks, :vcr do
  ...
  describe '#period' do
    ...
    it 'can be specified via option' do
      expect(Notu::MostPlayedTracks.new(library, period: '3month').period).to eq('3month')
    end

    it 'can be specified via option (as string)' do
      expect(Notu::MostPlayedTracks.new(library, 'period' => '12month').period).to eq('12month')
    end

    it 'raise an error if invalid' do
      expect {
        Notu::MostPlayedTracks.new(library, period: '42month')
      }.to raise_error(ArgumentError, 'Notu::MostPlayedTracks#period is invalid: "42month"')
    end
